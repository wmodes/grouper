...
# the file that the raw data is in
file="data/cst236s20.csv"
...
pref="data/cst236s20-pref.csv"

prefweight=-100
prefmatchweight=100

# an internal file to prep the csv
fileprep="data/cst236s20-prep.csv"
...
# make columns
# columns 1-23 schedule
for i in $(seq 1 23); do 
    measure+="diff "
    weight+="$sched_weight "
done
# columns 24-41 interests
for i in $(seq 24 41); do 
    measure+="diff "
    weight+="$interest_weight "
done
# columns 42-43 expertise
for i in $(seq 42 43); do 
    measure+="diff "
    weight+="$expert_weight "
done
# column 44 leadership role
measure+="diff "
weight+="$leader_weight "
# column 45 individual work
measure+="diff "
weight+="$group_weight "
# column 46 social
measure+="diff "
...
# Check for the unlikely event of blank fields, replace w 1 (:%s/,,/,1,/g)
