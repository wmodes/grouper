...
# get argument (format "s22")
if [ "$2" == "" ]; then
	echo "Usage: $0 term groups [pref]"
	echo "where:"
	echo "   term: the current term, e.g., f21, s22"
	echo "   groups: number of groups (has to be a power of 2)"
	echo "   pref: any value indicates a pref file has been written in data dir"
	echo "      Pref file format is: Dominic Jones,Hannah McAllister,-100"
	echo "Requirements:"
	echo "   * A survey file exists in the data dir corresponding to the term, e.g., art101s22"
	echo "   * A pref file exists in the data dir for this term, e.g., art101s22-pref"
	exit 1
fi
term="$1"
numgroups="$2"
